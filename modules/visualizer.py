...
class BaseBlock:
    ...
    def _local_electroshock(self, net, t: float, threshold_rate: float = 0.02):
        """
        Si el spike_rate cae bajo el umbral, inyecta spikes solo
        en las neuronas inmortales. NO toca el resto de la red.
        Mucho más quirúrgico que el electroshock global actual.
        """
...
BLOCK_REGISTRY = {
    ...
    # futuro: "detector_bordes": EdgeDetectorBlock,
    # futuro: "combinador_local": LocalCombinerBlock,
}
...
def create_block(block_type: str, **kwargs) -> Optional[BaseBlock]:
    ...
    if cls is None:
        print(f"⚠️ [Bloques] Tipo desconocido: '{block_type}'. Disponibles: {list(BLOCK_REGISTRY)}")
        return None
    return cls(**kwargs)
